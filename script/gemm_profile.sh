...
for args in "${ARGS_LIST[@]}"; do
  ...
  # Format: "The CPU veification result is:correct" (note typo in "veification")
  # Format: "The CPU verification result is: correct"
  VERIFICATION=$(echo "$OUTPUT" | grep -E "(GPU|CPU).*[vV]erification result is:" | sed -n 's/.*[vV]erification result is: *\([^[:space:]]*\).*/\1/p')
  
  # If not found with standard pattern, try the typo pattern
  if [ -z "$VERIFICATION" ]; then
    VERIFICATION=$(echo "$OUTPUT" | grep -E "(GPU|CPU).*[vV]eification result is:" | sed -n 's/.*[vV]eification result is: *\([^[:space:]]*\).*/\1/p')
  fi
