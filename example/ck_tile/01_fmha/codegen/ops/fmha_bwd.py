...
FMHA_BWD_KERNEL_INFO_MAP = {
    ...
    FMHA_BWD_CONVERT_DQ: {
        'trait_symbol' : 'fmha_bwd_convert_dq_traits_<{F_hdim}, {F_dtype}, {F_mode}, {F_spad1}, {F_dpad}, {F_deterministic}>',
...
class FmhaBwdDQDKDVApiTrait:
    ...
    def dq_reduce_check(self) -> str:
        ...
        elif self.atomic32 == 't' :    return '!t.is_deterministic && t.atomic32'
        else :                         return '!t.is_deterministic && !t.atomic32'

    def get_kernel_group(self) -> Tuple[str]:
        kernel_group = [FMHA_BWD_DOT_DO_O, FMHA_BWD_DQ_DQ_DV]
        if self.deterministic or self.atomic32:
    ...
    def api(self) -> str:  
        ...
        def get_dispatch_api_fuc_define(kernel_group : tuple) -> str:
            ...
            for kernel in kernel_group:
                ...
                template_list.append('typaname ' + trait_name)
                kernel_launch_list.append(pad_space_left(FMHA_BWD_KERNEL_INFO_MAP[kernel]['kernel_launch'].format(trait_name), 8))
                get_name_call_list.append(FMHA_BWD_KERNEL_INFO_MAP[kernel]['get_name_call'].format(trait_name))
        ...
        def get_inner_body(kernel_group : tuple, kernel_trait_dict : dict) -> str:
            ...
            for kernel in kernel_group:
                trait_name = kernel + '_trait_'
                body_lines.append(trait_define.format(F_trait_name=kernel, F_kernel_trait=kernel_trait_dict[kernel]))
