...
// Extended GemmHostArgs structure to support workspace
struct GemmSplitKHostArgs : public ck_tile::GemmHostArgs
{
    ...
    CK_TILE_HOST GemmSplitKHostArgs(const void* a_ptr_,
                                   ...
                                   bool use_reduction_mode_ = true)
        : BaseArgs(a_ptr_, b_ptr_, workspace_ptr_, k_batch_, M_, N_, K_, // pass the workspace_ptr_ to BaseArgs e_ptr
                   stride_A_, stride_B_, workspace_stride_, use_reduction_mode_),
          final_output_ptr(e_ptr_), final_stride_E(stride_E_)
    {
    }

    void* final_output_ptr;
    ck_tile::index_t final_stride_E;
};

// Stage 1: GEMM kernel that writes partial results to workspace
...
float gemm_stage1(const GemmSplitKHostArgs& args, const ck_tile::stream_config& s)
{
    ...
    // Create base args pointing to workspace instead of final output
    ck_tile::GemmHostArgs base_args(args.a_ptr, args.b_ptr, args.e_ptr, 
                                      args.k_batch, args.M, args.N, args.K,
                                      args.stride_A, args.stride_B, args.stride_E,
                                      true); // Enable reduction mode for split-K
    ...
    const auto RunSplitk = [&](const auto has_hot_loop_, const auto tail_number_) {
        // For reduction mode, always use SET operation since each split writes to separate memory
        Run(has_hot_loop_,
            tail_number_,
...
// Stage 2: Reduction kernel that sums partial results from workspace to final output
template <typename CDataType, typename ComputeDataType = float, typename ELayout = ck_tile::tensor_layout::gemm::RowMajor>
float reduce_stage2(const GemmSplitKHostArgs& args, const ck_tile::stream_config& s)
{
    ...
    auto workspace_strides = ck_tile::make_tuple(args.M * args.N,  // k_batch stride: jump to next K split
    ...
    using Problem = ck_tile::Reduce2dProblem<CDataType, ComputeDataType, CDataType, Shape, ReduceOp>;
    using Kernel = ck_tile::Reduce<Problem>;

    if(!Kernel::IsSupportedArgument(reduce_dim_size))
    {
...
// Main two-stage GEMM+SplitK function
template <typename GemmConfig,
          typename ADataType,
          typename BDataType,
          typename DsDataType,
...
float gemm_splitk_two_stage(const GemmSplitKHostArgs& args, const ck_tile::stream_config& s)
{
    ...
    if(s.log_level_ > 0)
    {
        ...
        std::cout << "Workspace size: " << args.k_batch << " x " << args.M << " x " << args.N 
                  << " = " << args.k_batch * args.M * args.N * sizeof(CDataType) << " bytes" << std::endl;
    }

    // Stage 1: GEMM to workspace
    gemm_time = gemm_stage1<GemmConfig, ADataType, BDataType, DsDataType, AccDataType, CDataType,
                            ALayout, BLayout, DsLayout, ELayout, Persistent, CDEElementWise>(args, s);
    ...
    // Stage 2: Reduction from workspace to final output
    if(args.k_batch > 1)
    {
        using ComputeDataType = std::conditional_t<std::is_same_v<CDataType, ck_tile::half_t>, float,
    ...
    {
        // If k_batch == 1, just copy from workspace to final output
        auto copy_result = hipMemcpyAsync(args.final_output_ptr, args.e_ptr, 
                      args.M * args.N * sizeof(CDataType), 
                      hipMemcpyDeviceToDevice, s.stream_id_);
...
// New invoke function for two-stage GEMM+SplitK
template <typename GemmConfig,
          typename ADataType,
          typename BDataType,
          typename DsDataType,
                                   ...
                                   bool persistent)
{
    ...
    GemmSplitKHostArgs args{
        ...
        kbatch,                               // k_batch
        M, N, K,                              // dimensions
        stride_A, stride_B,                   // input strides
    ...
    std::cout << "Run Two-Stage GEMM+SplitK with M=" << M << " N=" << N << " K=" << K
              ...
              << " StrideC=" << stride_C << " kbatch=" << kbatch
              << " WorkspaceSize=" << workspace_size << " bytes"
                                            ...
                                            [[maybe_unused]] const CLayout c_layout = CLayout{})
{
    ...
    bool split_k_mode          = arg_parser.get_int("split_k_mode");
    ...
    if(split_k_mode != 1)
    {
        std::cout << "This example implements Split-K reduction and not Split-K atomic!." << std::endl;
        return false;
    }
...
int run_gemm_example_prec_type(std::string a_layout, std::string b_layout, int argc, char* argv[])
{
    ...
    if constexpr(std::is_same_v<BPrecType, ck_tile::pk_int4_t>)
    {
        ...
        {
            return run_gemm_example_with_layouts_two_stage<GemmConfig, APrecType, BPrecType, CPrecType, Row, Col, Row>(
                argc, argv, Row{}, Col{}, Row{});
        }
        else if(a_layout == "C" && b_layout == "C")
        {
            return run_gemm_example_with_layouts_two_stage<GemmConfig, APrecType, BPrecType, CPrecType, Col, Col, Row>(
    ...
    else
    {
        ...
        {
            return run_gemm_example_with_layouts_two_stage<GemmConfig, APrecType, BPrecType, CPrecType>(
